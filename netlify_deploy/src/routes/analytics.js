...
router.get('/summary', requireAdmin, async (req, res) => {
    try {
        ...
        const { rows: slots } = await db.query(`
            ...
            WHERE s.is_active = 1
            GROUP BY s.id, s.name, s.start_time, s.end_time, s.capacity
        ...
        const { rows: trend } = await db.query(`
            ...
            WHERE b.date >= $1 AND b.date <= $2 AND b.status = 'confirmed'
            GROUP BY b.date
            ORDER BY b.date ASC
...
router.get('/categories', requireAdmin, async (req, res) => {
    try {
        ...
        catStart.setDate(catStart.getDate() - daysBack);
        const catStartStr = catStart.toISOString().split('T')[0];
...
router.get('/students', requireAdmin, async (req, res) => {
    try {
        ...
        const { rows: blockedRows } = await db.query("SELECT COUNT(*) as cnt FROM users WHERE role = 'student' AND blocked_until > $1", [nowStr]);
